...
class AttentionGraphModel(nn.Module):
    """
    Graph Attention Network as implemented in https://arxiv.org/pdf/1710.10903.pdf
    """
    ...
    def __init__(self, g, n_layers, input_size, hidden_size, output_size, nonlinearity):
        ...
        self.g = g
        self.layers = nn.ModuleList()
        self.layers.append(GATConv(input_size, hidden_size, 4, activation=nonlinearity))
        for i in range(n_layers-1):
            self.layers.append(GATConv(hidden_size * 4, hidden_size, 4, activation=nonlinearity, residual=True))
        self.layers.append(GATConv(hidden_size * 4, output_size, 6))
...
def main(args):
    ...
    # Setting the same LR as the GAT paper
